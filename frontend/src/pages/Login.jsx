...
import { LogIn, Eye, EyeOff, Shield, Mail, Zap } from 'lucide-react'
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail || 'Invalid credentials. Check your username and password.'
      toast.error(msg)
    } finally { setLoading(false) }
  }

  // One-click demo login — fills form AND signs in automatically
  const handleTryDemo = async () => {
    ...
    try {
      ...
      // If demo creds fail show a helpful message
      const msg = err?.response?.data?.detail || 'Demo sign-in failed — make sure the backend is running.'
      toast.error(msg, { duration: 5000 })
      setForm({ email: 'admin', password: 'admin123' })
    } finally { setDemoLoading(false) }
  }

  const fillDemo = () => setForm({ email: 'admin', password: 'admin123' })
    ...
    <div className="login-page">
      ...
      <div className="login-layout">
        ...
        <motion.div className="login-card"
            ...
            <AnimatePresence mode="wait">
              {demoLoading
                ? <motion.span key="l" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
                    ...
                    <div className="login-spinner" style={{borderTopColor:'var(--amber)'}}/>
                    Signing in to demo…
                  </motion.span>
                : <motion.span key="i" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
                    style={{display:'flex',alignItems:'center',gap:8}}>
                    <Zap size={15} fill="currentColor"/> Try the live demo
                  </motion.span>
              }
            </AnimatePresence>
          </motion.button>
          ...
          <motion.div className="social-buttons"
            ...
            {SOCIAL.map((p) => (
              <motion.button key={p.id} type="button" className="social-btn"
                ...
                onClick={() => handleSocial(p)} disabled={!!socialLoading || loading || demoLoading}
                whileHover={{ scale:1.02, y:-1 }} whileTap={{ scale:0.97 }}>
                <AnimatePresence mode="wait">
                  {socialLoading === p.id
          ...
          {/* Divider */}
          <motion.div className="auth-or-divider"
          ...
          <form onSubmit={handleLogin}>
            ...
            <motion.div className="form-group"
              initial={{ opacity:0, x:16 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.54 }}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                <label className="form-label login-label">Password</label>
                <span className="auth-forgot" onClick={() => toast('Password reset coming soon', {icon:'🔑'})}>
                  Forgot password?
                </span>
              </div>
          ...
          {/* Demo credentials bar — clickable to fill */}
          <motion.button
            type="button"
            ...
            <span className="login-demo-hint">← click to fill</span>
          </motion.button>
        </motion.div>
      </div>
    </div>
