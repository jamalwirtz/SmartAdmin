...
function Sidebar() {
  ...
  return (
    <motion.aside className="sidebar" initial={{ x: -240 }} animate={{ x: 0 }} transition={{ duration: 0.35, ease: [0.4,0,0.2,1] }}>
      ...
      >
        <span style={{fontSize:13,fontWeight:800,lineHeight:1}}>?</span> How it works
      </button>
    </motion.aside>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Show walkthrough once, on first login — never on the login/signup pages
    ...
    return () => {
      ...
      if (loginHandler) {
        window.removeEventListener('sstg_logged_in', loginHandler)
      }
    }
  }, [])
  const { user, loading } = useAuth()
